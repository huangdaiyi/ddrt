...
{get_user, <<"SELECT u.id, u.email,u.`type`,gu.receive_type,g.`name` AS gname,g.template,d.`name` AS dname FROM users AS u
...
{get_user_by_email, <<"SELECT u.id, u.email,u.`type`,gu.receive_type,g.`name` AS gname,g.template,d.`name` AS dname FROM users AS u
INNER JOIN groups_users AS gu ON u.id=gu.user_id INNER JOIN groups AS g ON gu.group_id=g.id LEFT JOIN
...
{get_all_reports, <<"SELECT u.id AS user_id, u.user_name, u.email, r.content, r.`date`, g.`name` AS group_name, g.template, gu.receive_type, 
d.`name` AS domain_name, d.id AS domain_id 
					FROM  users AS u 
